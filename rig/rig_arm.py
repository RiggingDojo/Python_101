...
class Rig_Arm:
    def rig_arm(self):
        ...
        self.arm_createJnt("IK_")
        self.arm_createJnt("FK_")
        self.arm_createJnt("bn_")

        vecList = ([1, 0, 0], [-1, 0, 0], [0, 1, 0], [0, -1, 0])
        self.arm_orientJoint("bn_", rigData["ArmJoints"][0][0], rigData["ArmJoints"][0][1], vecList[0], vecList[2])
        self.arm_orientJoint("bn_", rigData["ArmJoints"][0][1], rigData["ArmJoints"][0][2], vecList[0], vecList[2])

        self.arm_orientJoint("IK_", rigData["ArmJoints"][0][0], rigData["ArmJoints"][0][1], vecList[0], vecList[2])
        self.arm_orientJoint("IK_", rigData["ArmJoints"][0][1], rigData["ArmJoints"][0][2], vecList[0], vecList[2])

        self.arm_orientJoint("FK_", rigData["ArmJoints"][0][0], rigData["ArmJoints"][0][1], vecList[0], vecList[2])
        self.arm_orientJoint("FK_", rigData["ArmJoints"][0][1], rigData["ArmJoints"][0][2], vecList[0], vecList[2])

        self.arm_orientJoint("IK_", rigData["ArmJoints"][1][0], rigData["ArmJoints"][1][1], vecList[1], vecList[3])
        self.arm_orientJoint("IK_", rigData["ArmJoints"][1][1], rigData["ArmJoints"][1][2], vecList[1], vecList[3])

        self.arm_orientJoint("bn_", rigData["ArmJoints"][1][0], rigData["ArmJoints"][1][1], vecList[1], vecList[3])
        self.arm_orientJoint("bn_", rigData["ArmJoints"][1][1], rigData["ArmJoints"][1][2], vecList[1], vecList[3])

        self.arm_orientJoint("FK_", rigData["ArmJoints"][1][0], rigData["ArmJoints"][1][1], vecList[1], vecList[3])
        self.arm_orientJoint("FK_", rigData["ArmJoints"][1][1], rigData["ArmJoints"][1][2], vecList[1], vecList[3])

        self.arm_cleanOrient("IK_")
        self.arm_cleanOrient("FK_")
        self.arm_cleanOrient("bn_")


        self.arm_IK_Create()
        self.arm_IK_CtrlMaker()
        ...
        self.arm_PV_CtrlShape()
        self.arm_PV_CtrlMaker(lf_pvctrlinfo, rt_pvctrlinfo)
        self.arm_FK_CtrlShape()
        self.arm_FK_CtrlMaker()
        self.arm_StretchyIK()
        self.arm_IaF_Switch_CtrlShape()
        self.arm_IaF_Switch_CtrlMaker()
        self.arm_IaF_Switch()
    ...
    def arm_createJnt(self, jntType):
        for i in range(len(rigData["ArmJoints"])):
            for o in range(len(rigData["ArmJoints"][i])):
                cmds.joint(n=jntType+rigData["ArmJoints"][i][o], p=rigData["ArmJointsPos"][i][o])
                cmds.select(cl=True)
    ...
    def arm_cleanOrient(self, jntType):
        for i in range(len(rigData["ArmJoints"])):
    ...
    def arm_IK_CtrlMaker(self):
        for i in range(len(rigData["ArmJointsPos"])):
            pos = rigData["ArmJointsPos"][i][2]
    ...
    def arm_FK_CtrlMaker(self):     
        for o in range(len(rigData["ArmJointsPos"])):
            for s in range(len(rigData["ArmJointsPos"][o])):
                pos = rigData["ArmJointsPos"][o][s]
    ...
    def arm_PV_CtrlMaker(self, lf_position, rt_position):
        for i in range(len(rigData["IK_ArmList"])):
            if i==0:
                pos = lf_position
            else:
    ...
    def arm_IaF_Switch_CtrlMaker(self):
    ...
    def arm_IaF_Switch_CtrlShape(self):
        for i in range(len(rigData["IaF_Switch"])):
            crv = cmds.curve( degree = 1,\
    ...
    def arm_StretchyIK(self):
        for i in range(len(rigData["arm_StretchyIK"])):
            ...
            cmds.shadingNode("multiplyDivide", asUtility=True, n=rigData["arm_StretchyIK"][i][0])
            cmds.shadingNode("multiplyDivide", asUtility=True, n=rigData["arm_StretchyIK"][i][1])
            cmds.shadingNode("multiplyDivide", asUtility=True, n=rigData["arm_StretchyIK"][i][2])

            cmds.shadingNode("multDoubleLinear", asUtility=True, n=rigData["arm_StretchyIK"][i][3])
            cmds.shadingNode("multDoubleLinear", asUtility=True, n=rigData["arm_StretchyIK"][i][4])
            cmds.shadingNode("multDoubleLinear", asUtility=True, n=rigData["arm_StretchyIK"][i][5])

            cmds.shadingNode("condition", asUtility=True, n=rigData["arm_StretchyIK"][i][6])
            cmds.shadingNode("condition", asUtility=True, n=rigData["arm_StretchyIK"][i][7])
            cmds.shadingNode("condition", asUtility=True, n=rigData["arm_StretchyIK"][i][8])
            cmds.shadingNode("condition", asUtility=True, n=rigData["arm_StretchyIK"][i][9])

            cmds.shadingNode("blendTwoAttr", asUtility=True, n=rigData["arm_StretchyIK"][i][10])
            cmds.shadingNode("blendTwoAttr", asUtility=True, n=rigData["arm_StretchyIK"][i][11])

            cmds.shadingNode("blendColors", asUtility=True, n=rigData["arm_StretchyIK"][i][12])
            cmds.shadingNode("blendColors", asUtility=True, n=rigData["arm_StretchyIK"][i][13])

            if i == 1:
                cmds.shadingNode("multDoubleLinear", asUtility=True, n=rigData["arm_StretchyIK"][i][14])
                cmds.shadingNode("multDoubleLinear", asUtility=True, n=rigData["arm_StretchyIK"][i][15])
            else:
                pass
            cmds.setAttr(rigData["arm_StretchyIK"][i][0]+".operation", 2)
            cmds.setAttr(rigData["arm_StretchyIK"][i][1]+".operation", 2)
            cmds.setAttr(rigData["arm_StretchyIK"][i][2]+".operation", 2)

            cmds.setAttr(rigData["arm_StretchyIK"][i][6]+".operation", 3)
            cmds.setAttr(rigData["arm_StretchyIK"][i][7]+".operation", 3)
            cmds.setAttr(rigData["arm_StretchyIK"][i][8]+".operation", 3)
            cmds.setAttr(rigData["arm_StretchyIK"][i][9]+".operation", 3)

            cmds.connectAttr(elbowDist+".distance", rigData["arm_StretchyIK"][i][0]+".input1X")
            cmds.connectAttr(wristDist+".distance", rigData["arm_StretchyIK"][i][1]+".input1X")
            cmds.connectAttr(shldDist+".distance", rigData["arm_StretchyIK"][i][2]+".input1X")

            cmds.connectAttr(rigData["arm_StretchyIK"][i][3]+".output", rigData["arm_StretchyIK"][i][2]+".input2X")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][2]+".outputX", rigData["arm_StretchyIK"][i][4]+".input1")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][2]+".outputX", rigData["arm_StretchyIK"][i][5]+".input1")

            cmds.connectAttr(rigData["arm_StretchyIK"][i][0]+".outputX", rigData["arm_StretchyIK"][i][6]+".colorIfTrueR")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][1]+".outputX", rigData["arm_StretchyIK"][i][7]+".colorIfTrueR")

            cmds.connectAttr(shldDist+".distance", rigData["arm_StretchyIK"][i][8]+".firstTerm")
            cmds.connectAttr(shldDist+".distance", rigData["arm_StretchyIK"][i][9]+".firstTerm")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][3]+".output", rigData["arm_StretchyIK"][i][8]+".secondTerm")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][3]+".output", rigData["arm_StretchyIK"][i][9]+".secondTerm")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][4]+".output", rigData["arm_StretchyIK"][i][8]+".colorIfTrueR")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][5]+".output", rigData["arm_StretchyIK"][i][9]+".colorIfTrueR")

            if i == 1:
                cmds.connectAttr(rigData["arm_StretchyIK"][i][6]+".outColorR", rigData["arm_StretchyIK"][i][14]+".input1")
                cmds.connectAttr(rigData["arm_StretchyIK"][i][7]+".outColorR", rigData["arm_StretchyIK"][i][15]+".input1")
                cmds.connectAttr(rigData["arm_StretchyIK"][i][14]+".output", rigData["arm_StretchyIK"][i][10]+".input[1]")
                cmds.connectAttr(rigData["arm_StretchyIK"][i][15]+".output", rigData["arm_StretchyIK"][i][11]+".input[1]")
            else:
                cmds.connectAttr(rigData["arm_StretchyIK"][i][6]+".outColorR", rigData["arm_StretchyIK"][i][10]+".input[1]")
                cmds.connectAttr(rigData["arm_StretchyIK"][i][7]+".outColorR", rigData["arm_StretchyIK"][i][11]+".input[1]")

            cmds.connectAttr(rigData["arm_StretchyIK"][i][8]+".outColorR", rigData["arm_StretchyIK"][i][10]+".input[0]")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][9]+".outColorR", rigData["arm_StretchyIK"][i][11]+".input[0]")

            cmds.connectAttr(rigData["arm_StretchyIK"][i][10]+".output", rigData["arm_StretchyIK"][i][12]+".color1R")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][11]+".output", rigData["arm_StretchyIK"][i][13]+".color1R")
            ...
            cmds.setAttr(rigData["arm_StretchyIK"][i][3]+".input1", uparm_dist+lowarm_dist)
            cmds.setAttr(rigData["arm_StretchyIK"][i][6]+".secondTerm", 1)
            cmds.setAttr(rigData["arm_StretchyIK"][i][7]+".secondTerm", 1)
            cmds.setAttr(rigData["arm_StretchyIK"][i][6]+".colorIfFalseR", uparm_len)
            cmds.setAttr(rigData["arm_StretchyIK"][i][7]+".colorIfFalseR", lowarm_len)

            cmds.setAttr(rigData["arm_StretchyIK"][i][4]+".input2", uparm_len)
            cmds.setAttr(rigData["arm_StretchyIK"][i][5]+".input2", lowarm_len)

            cmds.setAttr(rigData["arm_StretchyIK"][i][8]+".colorIfFalseR", uparm_len)
            cmds.setAttr(rigData["arm_StretchyIK"][i][9]+".colorIfFalseR", lowarm_len)

            cmds.setAttr(rigData["arm_StretchyIK"][i][12]+".color2R", uparm_len)
            cmds.setAttr(rigData["arm_StretchyIK"][i][13]+".color2R", lowarm_len)
            if i == 1:
                cmds.setAttr(rigData["arm_StretchyIK"][i][14]+".input2", -1)
                cmds.setAttr(rigData["arm_StretchyIK"][i][15]+".input2", -1)

            cmds.connectAttr(rigData["HierarchyOrg"][12]+".CharScale", rigData["arm_StretchyIK"][i][3]+".input2")
            cmds.connectAttr(rigData["HierarchyOrg"][12]+".CharScale", rigData["arm_StretchyIK"][i][0]+".input2X")
            cmds.connectAttr(rigData["HierarchyOrg"][12]+".CharScale", rigData["arm_StretchyIK"][i][1]+".input2X")
            cmds.connectAttr(rigData["HierarchyOrg"][12]+".CharScale", rigData["arm_StretchyIK"][i][6]+".firstTerm")
            cmds.connectAttr(rigData["HierarchyOrg"][12]+".CharScale", rigData["arm_StretchyIK"][i][7]+".firstTerm")

            cmds.rename(shldDistDim, rigData["arm_StretchyIK"][i][16])
            cmds.rename(elbowDistDim, rigData["arm_StretchyIK"][i][17])
            cmds.rename(wristDistDim, rigData["arm_StretchyIK"][i][18])
            cmds.rename(shldDistLoc[0], rigData["arm_StretchyIK"][i][19])
            cmds.rename(elbowDistLoc[1], rigData["arm_StretchyIK"][i][20])
            cmds.rename(shldDistLoc[1], rigData["arm_StretchyIK"][i][21])
            cmds.connectAttr(rigData["arm_StretchyIK"][i][12]+".outputR", "IK_"+rigData["ArmJoints"][i][1]+".tx")
            cmds.connectAttr(rigData["arm_StretchyIK"][i][13]+".outputR", "IK_"+rigData["ArmJoints"][i][2]+".tx")


            cmds.pointConstraint(rigData["IK_ArmList"][i][0], rigData["arm_StretchyIK"][i][21])
            cmds.pointConstraint(rigData["IK_ArmList"][i][4], rigData["arm_StretchyIK"][i][20])

            cmds.connectAttr(rigData["IK_ArmList"][i][4]+".ElbowPin", rigData["arm_StretchyIK"][i][10]+".attributesBlender")
            cmds.connectAttr(rigData["IK_ArmList"][i][4]+".ElbowPin", rigData["arm_StretchyIK"][i][11]+".attributesBlender")
            cmds.connectAttr(rigData["IK_ArmList"][i][0]+".Stretch", rigData["arm_StretchyIK"][i][12]+".blender")
            cmds.connectAttr(rigData["IK_ArmList"][i][0]+".Stretch", rigData["arm_StretchyIK"][i][13]+".blender")
            if i == 0:
            ...
            cmds.parent(rigData["arm_StretchyIK"][i][16], ctrlGRP)
            cmds.parent(rigData["arm_StretchyIK"][i][17], ctrlGRP)
            cmds.parent(rigData["arm_StretchyIK"][i][18], ctrlGRP)
            cmds.parent(rigData["arm_StretchyIK"][i][19], ctrlGRP)
            cmds.parent(rigData["arm_StretchyIK"][i][20], ctrlGRP)
            cmds.parent(rigData["arm_StretchyIK"][i][21], ctrlGRP)


    def arm_IaF_Switch(self):
        for i in range(len(rigData["IaF_Switch"])):
